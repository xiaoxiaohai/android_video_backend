...
function buildWatchCandidates(watchUrl) {
  ...
  // Keep high-success hosts; drop hosts that frequently fail TLS/availability.
  const fallbackHosts = ['f2movies.la', 'f2moviesz.uk'];
...
function supports(watchUrl) {
  try {
    ...
    return /f2movies|f2moviesz/.test(host);
  } catch {
    return false;
  }
}
...
export const f2moviesProvider = { name: 'f2movies', supports, resolve };
